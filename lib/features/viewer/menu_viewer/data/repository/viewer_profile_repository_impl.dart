import 'dart:convert';
import 'dart:io';

import 'package:dartz/dartz.dart';
import 'package:dio/dio.dart';

import '../../../../../../core/app_config/app_urls.dart';
import '../../../../../../core/app_config/prefs_keys.dart';
import '../../../../../../core/error/exceptions.dart';
import '../../../../../../core/helpers/secure_local_storage.dart';
import '../../../../../../core/helpers/user_helper.dart';
import '../../../../../../core/network/network_request.dart';
import 'package:aflam/features/auth/login/data/model/response/user_model.dart';
import '../../../../auth/login/data/model/response/profile_model.dart';
import 'viewer_profile_repository.dart';

class ViewerProfileRepositoryImpl extends ViewerProfileRepository {
  @override
  Future<Either<CustomException, UserModel>> updateProfile({
    required String fullname,
    File? profilePhoto,
    String? personalInfo,
    required bool isAvailable,
  }) async {
    final result = await exceptionHandler(() async {
      final Map<String, dynamic> data = {
        'fullname': fullname,
        'is_available': isAvailable.toString(),
      };

      if (personalInfo != null) {
        data['personal_info'] = personalInfo;
      }

      if (profilePhoto != null) {
        data['profile_photo'] = await MultipartFile.fromFile(
          profilePhoto.path,
          filename: profilePhoto.path.split('/').last,
        );
      }

      final formData = FormData.fromMap(data);

      final response = await dioService.callApi(
        NetworkRequest(
          AppUrls.updateProfile,
          method: RequestMethod.patch,
          isFormData: true,
          formDataBody: formData,
        ),
        mapper: (json) {
          if (json['user'] != null) {
            return UserModel.fromJson(json['user']);
          }
          return UserModel.fromJson(json);
        },
      );

      UserModel updatedUser = response;
      final currentUser = UserHelper.userNotifier.value;

      if (currentUser != null) {
        // Merge strategy:
        // Start with the NEW user data from response (which has the updated fields).
        // If the 'profile' object in response is missing or lacks specific fields, patch them from 'currentUser'.
        // Why? Because 'updateProfile' API often returns a 'User' serialization that might be less complete than 'getUserProfile' or 'login'.

        ProfileModel? mergedProfile = updatedUser.profile;

        // If response profile is null, fall back to current profile
        if (mergedProfile == null) {
          mergedProfile = currentUser.profile;
        } else if (currentUser.profile != null) {
          // Both exist, so merge deeply if needed.
          // We assume 'response' has the LATEST data for fields it provides.
          // But if it provides nulls/defaults for fields it doesn't track, we should keep 'currentUser' values.
          final oldProfile = currentUser.profile!;

          mergedProfile = mergedProfile.copyWith(
            // If response has empty lists but we had data, consider keeping it?
            // Risky if the user actually deleted them. But for 'Update Profile' (name/bio/avail), we don't expect it to wipe out images/videos.
            // CAUTION: If the API returns empty list vs null.
            // ProfileModel.fromJson initializes lists to [] if null or empty.
            // If the response truly misses these fields, they will be empty.

            // For safety in this specific "Edit Profile" context where we only touch basic info:
            favoriteCategories:
                (mergedProfile.favoriteCategories.isEmpty &&
                    oldProfile.favoriteCategories.isNotEmpty)
                ? oldProfile.favoriteCategories
                : mergedProfile.favoriteCategories,

            images:
                (mergedProfile.images == null || mergedProfile.images!.isEmpty)
                ? oldProfile.images
                : mergedProfile.images,

            videos:
                (mergedProfile.videos == null || mergedProfile.videos!.isEmpty)
                ? oldProfile.videos
                : mergedProfile.videos,

            // Keep stats if new one is null/zero and old one had value?
            // Usually stats are server-side. If server returns 0/null, maybe it failed to load stats.
            views: mergedProfile.views ?? oldProfile.views,
            // ... add other fields if necessary
          );
        }

        updatedUser = updatedUser.copyWith(
          profile: mergedProfile,
          // Also check top-level fields
          ratingMean: (updatedUser.ratingMean == null)
              ? currentUser.ratingMean
              : updatedUser.ratingMean,
          wallet: (updatedUser.wallet == '0.00' && currentUser.wallet != '0.00')
              ? currentUser.wallet
              : updatedUser.wallet,
          points: (updatedUser.points == 0 && currentUser.points != 0)
              ? currentUser.points
              : updatedUser.points,
          // Maintain token-like things if any? (Not here usually)
        );
      }

      await SecureLocalStorage.write(
        PrefsKeys.user,
        jsonEncode(updatedUser.toJson()),
      );

      UserHelper.setUser(updatedUser);

      return updatedUser;
    });

    return result;
  }
}
