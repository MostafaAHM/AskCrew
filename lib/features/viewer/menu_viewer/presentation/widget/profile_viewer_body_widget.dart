...
import 'package:aflam/features/viewer/menu_viewer/presentation/widget/verification_tile.dart';
import '../../../../shared/payment/data/repository/watermark_payment_repository.dart';
import '../../../../shared/payment/presentation/cubit/watermark_payment_cubit.dart';

class ProfileViewerBodyWidget extends StatelessWidget {
  const ProfileViewerBodyWidget({super.key});
                          ...
                          children: [
                            ...
                            if (isLoggedIn)
                              AnimatedSettingsTile(
                                ...
                                title: AppStrings.swapAccounts,
                                isHighlighted: true,
                                onTap: () =>
                                    context.pushNamed(Routes.swapAccounts),
                              ),
                            AnimatedSettingsTile(
                              index: 3,
                            ...
                            if (isLoggedIn)
                              AnimatedSettingsTile(
                                ...
                                title: AppStrings.favoriteArtwork,
                                onTap: () =>
                                    context.pushNamed(Routes.favoriteArtwork),
                              ),
                            if (isLoggedIn)
                              AnimatedSettingsTile(
                                index: 8,
                            ...
                            if (isLoggedIn)
                              ...
                              AnimatedSettingsTile(
                                index: 9,
                                iconPath: AppIcons
                                    .logOut, // Use appropriate icon for login if available
      ...
      child: BlocConsumer<LogoutCubit, LogoutState>(
        listener: (context, state) {
          if (state is LogoutLoading) {
            ...
            // Clear user helper (storage already cleared by repository)
            UserHelper.clear();
            // Clear dio headers
            ...
            // (Storage already cleared by repository in the try block)
            UserHelper.clear();
            getIt<DioService>().clearHeaders();
            if (context.mounted) {
              context.pop(); // Close dialog first
