...
import 'enterprise_edit_profile_state.dart';

class EnterpriseEditProfileCubit extends Cubit<EnterpriseEditProfileState> {
  ...
  }) : super(EnterpriseEditProfileInitial());
  ...
  void _emitWorksLoaded({bool isSearching = false}) {
    emit(
      EnterpriseEditProfileWorksLoaded(
  ...
  void init(UserModel currentUser) {
    print('=== EnterpriseEditProfileCubit - init ===');
  ...
  void pickImage(File file) {
    ...
    emit(EnterpriseEditProfilePhotoSelected(file));
  }

  void toggleAvailable(bool value) {
    isAvailable = value;
    emit(EnterpriseEditProfileInitial()); // Rebuild UI
  }
  ...
  void removeSelectedWorkItem(int id, String type) {
    print('=== removeSelectedWorkItem (Enterprise) ===');
  ...
  Future<void> updateProfile({
    ...
    emit(EnterpriseEditProfileLoading());

    print('=== EnterpriseEditProfileCubit - updateProfile ===');
    ...
    result.fold(
      (failure) {
        ...
        emit(EnterpriseEditProfileFailure(failure.message));
      },
      (user) async {
        ...
        emit(EnterpriseEditProfileSuccess(user));
      },
    );
  }
}
